...
expressApp.get("/dashboard/guild/:guildId/config", async (req, res) => { try { const session = req.session; if (!session) return res.redirect("/auth/discord"); const c = await settings.get(req.params.guildId).catch(() => null); res.send(`<!DOCTYPE html><html><head><style>${getStyles()}</style></head><body><div class="layout"><aside class="sidebar">${sidebar({ username: session.userId }, req.params.guildId)}<main class="main"><div class="topbar"><h2>Server Settings</h2></div><div class="card"><h3>Guild Configuration</h3>${c?`<p style="color:var(--text-muted)">Prefix: <strong>${c.prefix||'!'}</strong></p>`:'<p style="color:var(--text-muted)">No config found.</p>'}</div></main></div></body></html>`); } catch (e) { logger.error("dashboard", "settings page error", e); res.status(500).send(errorPage("Failed to load settings.")); } });
