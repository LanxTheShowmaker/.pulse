...
expressApp.get("/dashboard", async (req, res) => {
    try {
        ...
        res.send(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>.pulse Dashboard</title><style>${getStyles()}</style></head><body><div class="layout"><aside class="sidebar">${sidebar({ username: session.userId, avatar: "https://cdn.discordapp.com/embed/avatars/0.png" }, gid)}<main class="main"><div class="topbar"><h2>Dashboard</h2><form action="/auth/logout" method="POST" style="display:inline"><button type="submit" class="btn btn-secondary">Logout</button></form></div><div class="cards"><div class="card"><h3>Open Tickets</h3><div class="value">${stats.open||0}</div></div><div class="card"><h3>Closed</h3><div class="value">${stats.closed||0}</div></div><div class="card"><h3>Cases</h3><div class="value">${recentCases.length}</div></div></div><div class="table-container"><table><thead><tr><th>Ticket</th><th>Status</th><th>Priority</th></tr></thead><tbody>${openTickets.map(t => `<tr><td>#${t.id.slice(0,8)}</td><td><span class="badge badge-${t.status?.toLowerCase()||'open'}">${t.status||'OPEN'}</span></td><td>${t.priority||'NORMAL'}</td></tr>`).join('')}${openTickets.length===0?'<tr><td colspan="3" style="text-align:center;color:var(--text-muted);padding:2rem;">No open tickets</td></tr>':''}</tbody></table></div></main></div></body></html>`);
    } catch (e) {
        logger.error("dashboard", "render error", e);
        res.status(500).send(errorPage("Failed to load dashboard: " + e.message));
    }
});

expressApp.get("/dashboard/guild/:guildId/tickets", async (req, res) => { try { const session = req.session; if (!session) return res.redirect("/auth/discord"); const gt = await tickets.getOpenTicketsSummary(req.params.guildId, 50); res.send(`<!DOCTYPE html><html><head><style>${getStyles()}</style></head><body><div class="layout"><aside class="sidebar">${sidebar({ username: session.userId }, req.params.guildId)}<main class="main"><div class="topbar"><h2>Tickets</h2></div><div class="table-container"><table><thead><tr><th>ID</th><th>Status</th><th>Priority</th></tr></thead><tbody>${gt.map(t => `<tr><td>#${t.id.slice(0,8)}</td><td><span class="badge badge-${t.status?.toLowerCase()||'open'}">${t.status||'OPEN'}</span></td><td>${t.priority||'NORMAL'}</td></tr>`).join('')}${gt.length===0?'<tr><td colspan="3" style="text-align:center;color:var(--text-muted);padding:2rem;">No open tickets</td></tr>':''}</tbody></table></div></main></div></body></html>`); } catch (e) { logger.error("dashboard", "tickets page error", e); res.status(500).send(errorPage("Failed to load tickets.")); } });

expressApp.get("/dashboard/guild/:guildId/moderation/cases/recent", async (req, res) => { try { const session = req.session; if (!session) return res.redirect("/auth/discord"); const c = await moderation.getRecentCasesApi(req.params.guildId, 25); res.send(`<!DOCTYPE html><html><head><style>${getStyles()}</style></head><body><div class="layout"><aside class="sidebar">${sidebar({ username: session.userId }, req.params.guildId)}<main class="main"><div class="topbar"><h2>Moderation Cases</h2></div><div class="table-container"><table><thead><tr><th>Case</th><th>Target</th><th>Action</th><th>Moderator</th></tr></thead><tbody>${c.map(x => `<tr><td>#${x.caseNumber}</td><td>${x.targetTag}</td><td>${x.action}</td><td>${x.moderatorTag}</td></tr>`).join('')}${c.length===0?'<tr><td colspan="4" style="text-align:center;color:var(--text-muted);padding:2rem;">No cases found</td></tr>':''}</tbody></table></div></main></div></body></html>`); } catch (e) { logger.error("dashboard", "moderation page error", e); res.status(500).send(errorPage("Failed to load cases.")); } });

expressApp.get("/dashboard/guild/:guildId/logs/mod", async (req, res) => { try { const session = req.session; if (!session) return res.redirect("/auth/discord"); const h = await logging.getModLogHistory(req.params.guildId, 50); res.send(`<!DOCTYPE html><html><head><style>${getStyles()}</style></head><body><div class="layout"><aside class="sidebar">${sidebar({ username: session.userId }, req.params.guildId)}<main class="main"><div class="topbar"><h2>Moderation Log</h2></div><div class="table-container"><table><thead><tr><th>Case</th><th>Action</th><th>Moderator</th><th>Date</th></tr></thead><tbody>${h.map(x => `<tr><td>#${x.caseNumber||'—'}</td><td>${x.action||x.event||'—'}</td><td>${x.moderatorTag||'—'}</td><td>${x.createdAt?new Date(x.createdAt).toLocaleDateString():'—'}</td></tr>`).join('')}${h.length===0?'<tr><td colspan="4" style="text-align:center;color:var(--text-muted);padding:2rem;">No entries</td></tr>':''}</tbody></table></div></main></div></body></html>`); } catch (e) { logger.error("dashboard", "mod log page error", e); res.status(500).send(errorPage("Failed to load mod log.")); } });

expressApp.get("/dashboard/guild/:guildId/config", async (req, res) => { try { const session = req.session; if (!session) return res.redirect("/auth/discord"); const c = await settings.getGuildConfig(req.params.guildId).catch(() => null); res.send(`<!DOCTYPE html><html><head><style>${getStyles()}</style></head><body><div class="layout"><aside class="sidebar">${sidebar({ username: session.userId }, req.params.guildId)}<main class="main"><div class="topbar"><h2>Server Settings</h2></div><div class="card"><h3>Guild Configuration</h3>${c?`<p style="color:var(--text-muted)">Prefix: <strong>${c.prefix||'!'}</strong></p>`:'<p style="color:var(--text-muted)">No config found.</p>'}</div></main></div></body></html>`); } catch (e) { logger.error("dashboard", "settings page error", e); res.status(500).send(errorPage("Failed to load settings.")); } });
